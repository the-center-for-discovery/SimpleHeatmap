...
opacity_drkred = int(255 * degree_transparency_drkred) 
colorDrkRed = colorDrkRed + (opacity_drkred,)
...
for i in images:
    ...
    for key, value in coords.items():
        if key in risk_high_dr:
            data.append(key)
            overlay_drawing.ellipse(coords[key], fill=colorDrkRed)
            loc_risk = Image.alpha_composite(loc_image, overlay)
        elif key in risk_high:
            data.append(key)
...
def send_email(email_recipient,
    ...
    email_sender = 'canderson@tcfd.org'
    ...
    try:
        server = smtplib.SMTP('mail.tcfd.org', 587)
        server.ehlo()
        server.starttls()
        server.login('maincampus\canderson', 'Shamb1es06')
        text = msg.as_string()
...
send_email('covid19report@tcfd.org',
            'COVID 19 Heat Map',
            'Please see attached latest COVID 19 Heat Map.',
            'CV19 AUTO HEATMAP.pptx')

send_email('canderson@tcfd.org',
            'COVID 19 Heat Map',
            'Please see attached latest COVID 19 Heat Map.',
            'CV19 AUTO HEATMAP.pptx')
